  ...
  return (
    <div className="w-full max-w-xl mx-auto rounded-[20px] bg-red-200 flex flex-col items-center justify-center text-center px-6 py-10">
      ...
      <div className="w-full mt-4 text-left">
        <h3 className="text-lg font-medium mb-2">Danh sách phát:</h3>
        <ul className="space-y-1">
          {tracks.map((track, index) => (
            <li key={index}>
              <button
                onClick={() => playTrack(index)}
                className={`cursor-pointer w-full text-left px-4 py-2 rounded-md ${
                  index === currentTrack ? 'bg-pink-400 text-white' : 'bg-white text-gray-800'
                } transition`}
              >
                {track.title}
              </button>
